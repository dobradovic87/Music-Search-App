...
function renderNewPage() {
    ...
    if (pagesObj.hasOwnProperty('topalbums')) {
        for (var i = 0; i < pagesObj.topalbums.album.length; i++) {
            ...
            for (j = 0; j < pagesObj.topalbums.album[i].image.length; j++) {
                if ((pagesObj.topalbums.album[i].image[j]["#text"] !== "") && (pagesObj.topalbums.album[i].image[j].size === "extralarge")) {
                    img.setAttribute('src', pagesObj.topalbums.album[i].image[j]["#text"] + ":./images/no-image-found.jpg");
                }
                else {
                    img.setAttribute('src', './images/no-image-found.jpg');
                }
